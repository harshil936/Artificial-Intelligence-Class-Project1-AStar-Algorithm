...
public class MtStHelensExp_999121498_914392888 implements AIModule{
    
    // To enable a efficient behavior for the PriorityQueue
    public class NodeComparator implements Comparator<MtStHelensExp_999121498_914392888.points>, Serializable{  // For the hash map 
      public int compare(MtStHelensExp_999121498_914392888.points n1, MtStHelensExp_999121498_914392888.points n2){
      if(n1.f>n2.f)
        return 1;
      else if(n1.f < n2.f)
        return -1;
  ...
  public MtStHelensExp_999121498_914392888.points prev; // from where it came from
  public double g;
  public double f;
  ...
  public boolean equals(Object obj){
    final MtStHelensExp_999121498_914392888.points other = (MtStHelensExp_999121498_914392888.points) obj;
  ...
  public points(final TerrainMap map, Point point, double g){
    ...
    public List<Point> finalpath(MtStHelensExp_999121498_914392888.points node){
  ...
  PriorityQueue<MtStHelensExp_999121498_914392888.points> pQueue = new PriorityQueue<MtStHelensExp_999121498_914392888.points>(5,new NodeComparator()); 
  //Map<String, MtStHelensExp_999121498_914392888.points> openSet = new HashMap<String, MtStHelensExp_999121498_914392888.points>();
  Map<String, points> openl = new HashMap<String, points>();   // Open List   --------->   this is a queue
  //Map<String, MtStHelensExp_999121498_914392888.points> closedSet = new HashMap<String, MtStHelensExp_999121498_914392888.points>();
  Map<String, points> closel = new HashMap<String, points>();    // List to not look in again
  MtStHelensExp_999121498_914392888.points start = new MtStHelensExp_999121498_914392888.points(map, map.getStartPoint(), 0.0);
  MtStHelensExp_999121498_914392888.points end = new MtStHelensExp_999121498_914392888.points(map, map.getEndPoint(),0.0);
...
int index = 0;
  ...
  while(openl.size()>0){
      ...
      MtStHelensExp_999121498_914392888.points poppingelement = pQueue.poll();
      ...
      if(poppingelement.id.equals(end.id)){ // If we get to the goal we return the path
        ...
        while(counter < neighbors.length) {
            MtStHelensExp_999121498_914392888.points the_neighbor = new MtStHelensExp_999121498_914392888.points(map, neighbors[counter], map.getCost(poppingelement.pnt, neighbors[counter]));
  ...
  for (int i=0; i < neighbors.length; i++){
        ...
        MtStHelensExp_999121498_914392888.points visited = closel.get(neighbors[i].x+","+neighbors[i].y); 
          if (visited == null){  // If we havn't seen this coordinate before
           double tentative_score = poppingelement.g + map.getCost(poppingelement.pnt, neighbors[i]); // the cost that we calculate to check against the previous cost if available
            ...
            MtStHelensExp_999121498_914392888.points isopen = null;
            isopen = openl.get(neighbors[i].x+","+neighbors[i].y);
      
      
            // If the successor does not exist we add it with a cost
          if (isopen == null){ 
            isopen = new MtStHelensExp_999121498_914392888.points(map, neighbors[i], tentative_score);
